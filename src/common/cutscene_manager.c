...
void cutsceneMgr_createCutscene(cutsceneMgr* self) {
    ...
    if (BITS_HAS(cutscene_overlay, CUTSCENE_OVL_FADE)) {
    ...
    if (BITS_HAS(cutscene_overlay, CUTSCENE_OVL_FILM_REEL)) {
...
void cutsceneMgr_setCameraClippingAndScissoring(cutsceneMgr* self) {
    if (BITS_HAS(
            cutscene_settings[self->cutscene_ID - 1].overlay, CUTSCENE_OVL_WIDESCREEN_BORDERS
        )) {
...
void cutsceneMgr_loop(cutsceneMgr* self) {
    if (objectList_findFirstObjectByID(cutscene_settings[self->cutscene_ID - 1].object_ID) ==
        NULL) {
        if (BITS_HAS(cutscene_settings[self->cutscene_ID - 1].overlay, CUTSCENE_OVL_FILM_REEL)) {
...
void cutsceneMgr_stopCutscene(cutsceneMgr* self) {
    ...
    if (BITS_HAS(settings->overlay, CUTSCENE_OVL_FADE)) {
