...
void createGameState(s32 game_state) {
    ...
    // Set GameStateMgr params (flags, destroy function, game state ID and game state info)
    ptr_GameStateMgr->flags |= TOP;
    ptr_GameStateMgr->destroy = GameStateMgr_destroy;
    ptr_GameStateMgr->current_game_state = game_state;
    memory_copy(&gameState_settings[game_state - 1].info, &ptr_GameStateMgr->current_game_state_info, sizeof(gameState_info));
}
